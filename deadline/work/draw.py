...
class Grapher(object):
    ...
    def draw(self):
        ...
        try:
            ...
            print("Unable to use DOT layout: %s" % ex, file=sys.stderr)
...
class Func(object):
    ...
    def draw(self, fetch):
        ...
        print("Fetch: %s" % fetch.inst, file=sys.stdout)

        g = Grapher()
        
        for k, v in list(self.graph.repo.items()):
            if v.temp is None:
                info = "%d" % (v.perm.info)
            else:
    ...
    def show(self):
        ...
        print("Function: %s" % self.fid, file=sys.stdout)
