...
class App(object, metaclass=ABCMeta):
    ...
    def parse(self):
        ...
        with open(log) as f:
            for line in f:
                ...
                if result is None or len(result) != 2:
                    LOG_ERR("Opt parsing failed: %s %s" % (line, result))
                    #return False
                    continue
    ...
    def irgen(self, inputs, force):
        ...
        for r in runs:
            ...
            if exists(out) and not force:
                ...
                command = "%s -emit-llvm -g %s -o %s %s" % (LLVM_BIN_CLA, runs[r], out, r)
                if "O2" not in os.environ:
                    command = command.replace("-O2", "-O0").replace("-Os", "-O0")
                cmd = PoolWork(True, red, command)
    ...
    def result(self):
        ...
        for x in sorted(list(dset.items()), key = lambda x : len(x[1]), reverse = True):
