...
func (mgr *Manager) preloadCorpus() {
	...
	oldcorpusDB, err := db.Open(filepath.Join(mgr.cfg.Workdir, "corpus_old.db"), true)
	if err != nil {
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, []byte, error) {
	...
	if mgr.cfg.DistanceFile != "" {
		...
		if err != nil {
			...
			return nil, nil, fmt.Errorf("failed to copy distance.json: %v", err)
		}
	}
	...
	if mgr.cfg.PCCFGFile != "" {
		...
		if err != nil {
			...
			return nil, nil, fmt.Errorf("failed to copy cfg.json: %v", err)
		}
	}
	...
	if mgr.cfg.GFuzzFilter != ""{
		...
		if err != nil {
			...
			return nil, nil, fmt.Errorf("failed to copy gfuzzfilter.json: %v", err)
		}
	}
