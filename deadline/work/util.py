...
def LOG_ERR(msg, detail = None):
    print(colored(msg, "red"), file=sys.stdout)
    if detail is not None:
        print(detail, file=sys.stderr)

def LOG_WRN(msg, detail = None):
    print(colored(msg, "yellow"), file=sys.stdout)
    if detail is not None:
        print(detail, file=sys.stderr)

def LOG_INF(msg, detail = None):
    print(colored(msg, "green"), file=sys.stdout)
    if detail is not None:
        print(detail, file=sys.stderr)
...
def mkdirs(pn, force = True):
    if exists(pn):
        if not force:
            ans = input("%s exists, delete ? " % pn)
            if ans.lower() != "y":
                return False
